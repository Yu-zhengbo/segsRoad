...
from mmseg.models.backbones.mamba.spiralmamba.diagscan import FastDiagScanv2
...
class DASSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if dual:
            self.x_proj_fre = nn.Linear(self.d_inner, (self.dt_rank + self.d_state*2), bias=False, **factory_kwargs)
        ...
        if dual:
            self.dt_projs_fre = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, **factory_kwargs)
        ...
        if dual:
            self.A_logs_fre = self.A_log_init(self.d_state, self.d_inner, dt_init)
            self.Ds_fre = self.D_init(self.d_inner, dt_init)
            self.diag_scan = FastDiagScanv2(input_size,input_size)
            self.fre_scale = nn.Parameter(torch.ones(1,self.d_inner,1,1))
            # self.fre_norm = nn.GroupNorm(num_groups=16, num_channels = d_model * 2)
            self.fre_norm = nn.LayerNorm(self.d_inner)
    ...
    def fre_branch(self, x: torch.Tensor,B,C,H,W):
        with torch.cuda.amp.autocast(enabled=False):
            ...
            x_fre = dct_2d(x_fp32, norm='ortho')
            # 3. 螺旋扫描
            xs_fre = torch.cat([x_fre, x_fre.transpose(2,3)], dim=1).flatten(2) # -> [B, 2*C, L]
            xs_fre = self.diag_scan(xs_fre) # -> [B, 2*C, L]
            # 4. 频域SSM处理
            xs_fre = xs_fre.reshape(B*2, C, -1)
            xs_fre = self.fre_norm(xs_fre.permute(0,2,1)).permute(0,2,1)
            y_freq_2d = self.ssm_frequence(xs_fre) # -> [B*2, C, L]
            y_freq_2d = self.diag_scan.recover(y_freq_2d) # -> [B*2, C, H, W]
            y_freq_2d = y_freq_2d[:B,...] + y_freq_2d[B:, ...].transpose(2,3) # -> [B, C, H, W]
            # 6. 恢复复数
            y_frequency = idct_2d(y_freq_2d, norm='ortho')
        return y_frequency
...
class DualDiagMamba_base(DAMamba):
    def __init__(self, **kwargs):
        super().__init__(
