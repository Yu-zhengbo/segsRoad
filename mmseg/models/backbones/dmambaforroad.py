...
class DASSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj = nn.Linear(self.d_inner//self.group, (self.dt_rank + self.d_state*2), bias=False, **factory_kwargs)
        self.x_proj_weight = nn.Parameter(self.x_proj.weight)
        del self.x_proj

        self.dt_projs = self.dt_init(self.dt_rank, self.d_inner//self.group, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, **factory_kwargs)
        self.dt_projs_weight = nn.Parameter(self.dt_projs.weight)
        self.dt_projs_bias = nn.Parameter(self.dt_projs.bias)
        del self.dt_projs

        self.A_logs = self.A_log_init(self.d_state, self.d_inner//self.group, dt_init)
        self.Ds = self.D_init(self.d_inner//self.group, dt_init)
        ...
        self.da_scan = Dynamic_Adaptive_Scan(channels=self.d_inner//group,group=num_group)
    @staticmethod
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        input = input.reshape(B*self.group, -1, H, W)
...
class DAMamba(nn.Module):
    ...
    def init_weights(self, pretrained=None):
            ...
            for k, v in pretrained_dict.items():
                if k in model_dict.keys():
                    if v.shape==model_dict[k].shape:
                        ...
                        try:
                            temp_shape_div = torch.tensor(v.shape)/torch.tensor(model_dict[k].shape)
                            temp_index = torch.where(temp_shape_div==4)
        ...
        if isinstance(pretrained, str):
            ...
            temp_dict = transfer_state_dict(state_dict, model_dict)

            missing_keys, unexpected_keys = self.load_state_dict(temp_dict, strict=False)
