...
class TextureL1Loss(nn.Module):
    ...
    def forward(self,
        ...
        target_texture = self.sobel(target.float())

        loss_texture = self.loss_weight * nn.functional.l1_loss(
            pred_texture, target_texture, weight, reduction=reduction)
        return loss_texture
...
class DenoiseUNet(BaseModule):
    ...
    def loss(self,x, timesteps, data_samples):
        seg_logits = self.forward(x, timesteps)
        ...
        gt_temp = torch.stack(gt_semantic_segs, dim=0)
        seg_logits = resize(
            input=seg_logits,
            size=gt_temp.shape[2:],
            mode='bilinear',
            align_corners=False)
        losses = dict()
        losses['ce_loss'] = self.loss_ce(seg_logits,gt_temp.float())
        losses['texture_loss'] = self.loss_texture(seg_logits, gt_temp)
