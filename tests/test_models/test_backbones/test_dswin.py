...
def test_swin_transformer():
    ...
    with pytest.raises(TypeError):
        ...
        DSwinTransformer(pretrained=123)

    with pytest.raises(AssertionError):
        # Because swin uses non-overlapping patch embed, so the stride of patch
        # embed must be equal to patch size.
        DSwinTransformer(strides=(2, 2, 2, 2), patch_size=4)

    # test pretrained image size
    with pytest.raises(AssertionError):
        DSwinTransformer(pretrain_img_size=(112, 112, 112))

    # Test absolute position embedding
    temp = torch.randn((1, 3, 112, 112))
    model = DSwinTransformer(pretrain_img_size=112, use_abs_pos_embed=True)
    model.init_weights()
    model(temp)

    # Test patch norm
    model = DSwinTransformer(patch_norm=False)
    model(temp)

    # Test normal inference
    temp = torch.randn((1, 3, 256, 256))
    model = DSwinTransformer()
    outs = model(temp)
    ...
    # Test frozen
    model = DSwinTransformer(frozen_stages=4)
    model.train()
    for p in model.parameters():
        assert not p.requires_grad

    # Test absolute position embedding frozen
    model = DSwinTransformer(frozen_stages=4, use_abs_pos_embed=True)
    model.train()
    for p in model.parameters():
        assert not p.requires_grad
    ...
    model = DSwinTransformer(with_cp=True)
